...
def print_path_onwards(path, identation, file):
    ...
    for item in ordered_dir:
        ...
        if os.path.isdir(item_path):
            ...
            file.write("\n" + spacing(identation) + os.path.join(path, item))
            print_path_onwards(item_path, identation + 1, file)
        else:
            print(spacing(identation), item)
            file.write("\n" + spacing(identation) + item)

#absolute_path = os.path.join("C:", "\Program Files (x86)", "Steam", "steamapps", "common") # Example

absolute_path = input("Enter absolute path: ")

with open("paths.txt", "w", encoding="utf-8") as file:
    print_path_onwards(absolute_path, 0, file)